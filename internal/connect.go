...
func TcpConnect(srcConnCtx context.Context, opts net.TcpOptions, link *net.Connection) error {
	...
	logrus.Info("tcp dail: ", link.Destination)
	...
	defer func() {
		logrus.Infof("tcp dail-serve terminated, address: %s, %s ", link.Address, link.Destination)
		net.Close(dstTCPConn)
	}()
	if err := net.SetTcpOptions(dstTCPConn, opts); err != nil {
		return fmt.Errorf("tcp set dest options: %w", err)
	}
	dstCtx, dstCancel := context.WithCancel(srcConnCtx)
	defer dstCancel()
	errors := make(chan error, 2)
	copier := func(_ context.Context, name string, from, to net.Conn) {
		defer logrus.Info("tcp %s copier terminated, destination: ", name, link.Destination)
